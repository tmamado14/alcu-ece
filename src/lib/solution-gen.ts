...
async function generateAndStore(problemId: string): Promise<string> {
  ...
  const solution = await deepseekChat({ ...prompt, thinking: false, timeoutMs: 60_000 });
  if (!solution) throw new Error("DeepSeek returned an empty completion");
...
function buildPrompt(p: ProblemForPrompt): { system: string; user: string } {
  ...
  const system = [
    ...
    "Formatting rules (the renderer is limited):",
    "- Use $...$ for inline math and $$...$$ for display math (KaTeX).",
    "- The only other markup available is **bold** and blank lines between paragraphs. No headings, no bullet markers, no tables, no \\( \\) delimiters.",
