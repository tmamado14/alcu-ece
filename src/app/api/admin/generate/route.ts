...
export const POST = handle(async (req: Request) => {
  ...
  const system = [
    ...
    `{"statement": "...", ${ANSWER_DATA_SPEC[body.answerType]}, "difficulty": <int>, "cognitive_level": "recall|comprehension|application|analysis|synthesis|evaluation", "estimated_time": <seconds>, "tags": ["..."], "hints": ["one or two hints"], "solution": "worked solution", "explanation": "one-paragraph takeaway", "reference": "textbook or standard, or empty string"}`,
    "Rules:",
    ...
    "- Write math as $...$ (inline, one line) or $$...$$ (display) LaTeX, in statements, choices, hints, solutions, and explanations alike.",
    "- NEVER use the \\( \\) or \\[ \\] delimiters: the renderer does not support them and they appear as literal backslashes to the learner. Write $x$ instead of \\(x\\), and $$x$$ instead of \\[x\\].",
    "- The only other markup allowed is **bold**; no headings, bullets, tables, or LaTeX environments (align, equation, cases).",
